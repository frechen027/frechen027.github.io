...
!!! question "代码提示与类型检查"
    ...
    *   **默认情况**：Python 是动态类型的，编辑器在静态分析时不知道 `animal` 到底是什么类型，所以**通常不会提示**，也不会报错。如果你拼写错误写成了 `animal.spek()`，只有在程序**运行**到这一行时才会抛出 `AttributeError`。
    
    *   **类型注解 (Type Hints)**：为了获得更好的开发体验，现代 Python 推荐加上**类型提示**：
        ```python
